...
export default async function IngestPage() {
  ...
  if (!session?.profile || !hasRole(session.profile, WRITER_ROLES)) {
    return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-slate-500">Acceso no autorizado.</div>;
  }
  ...
  return (
    <AppShell userName={session.profile.full_name} role={session.profile.role}>
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-100">Carga de datos</h1>
          <p className="text-sm text-slate-500 mt-0.5">Manual o importación CSV con validación y trazabilidad</p>
        </div>
